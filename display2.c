...
void writeTextToSurface(display *d, char *message, int r, int g, int b)
{
  ...
  *d->box = (SDL_Rect) { 100, 100, 320, 320 };

  d->text = TTF_RenderText_Shaded(d->font,message,text_color, d->bg);
  if (d->text == NULL) {
    SDL_Fail("TTF_RenderText_Solid() Failed", d);
  }
  // if (SDL_BlitSurface(d->text, d->box, d->surface, d->box) != 0) {
  //   SDL_Fail("SDL_BlitSurface() Failed", d);
  // }
}
